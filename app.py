...
combined_df.columns = [col.strip().lower().replace(" ", "_").replace("#", "#") for col in combined_df.columns]

# Parse dates and create year_month
combined_df['local_date'] = pd.to_datetime(combined_df['local_date'], errors='coerce')
combined_df['year_month'] = combined_df['local_date'].dt.to_period('M').astype(str)
combined_df['year'] = combined_df['local_date'].dt.year.astype(str)
...
def update_graphs(selected_year):
    ...
    if filtered_df.empty:
        fig = px.line(title=f"No data found for {selected_year}")
